...
export default function Home() {
  ...
  // Listen for real-time online count updates via custom event
  useEffect(() => {
    const handleOnlineUpdate = (event: Event) => {
      const customEvent = event as CustomEvent;
      setWsOnlineCount(customEvent.detail);
    };

    window.addEventListener("online-count-updated", handleOnlineUpdate);
    return () => window.removeEventListener("online-count-updated", handleOnlineUpdate);
  }, []);
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-3">
            <div className="flex items-center gap-2 bg-violet-50 dark:bg-violet-900/20 px-4 py-2 rounded-full border border-violet-200 dark:border-violet-800">
              ...
              <span className="text-sm font-medium text-violet-600 dark:text-violet-400">{wsOnlineCount || onlineCount || 0}</span>
            </div>
          </div>
        </div>
      </div>
