...
export class FirestoreStorage implements IStorageFirebase {
  ...
  async createUser(insertUser: InsertUser): Promise<User> {
    ...
    const user: User = { id: userId, ...insertUser } as User;
  ...
  async createPack(insertPack: InsertPack): Promise<Pack> {
    ...
    const pack: Pack = { id: packId, views: 0, deletedAt: null, ...insertPack } as Pack;
    await firestore.collection('packs').doc(packId).set(pack);
    return pack;
  }

  async updatePack(id: string, pack: UpdatePack): Promise<Pack | undefined> {
    await firestore.collection('packs').doc(id).update(pack as any);
    return this.getPackById(id);
  }

  async softDeletePack(id: string): Promise<void> {
    await firestore.collection('packs').doc(id).update({ deletedAt: new Date().toISOString() });
  }

  async restorePack(id: string): Promise<void> {
    await firestore.collection('packs').doc(id).update({ deletedAt: null });
  }
  ...
  async permanentlyDeletePack(id: string): Promise<void> {
    ...
    for (const fc of flashcards) {
      batch.delete(firestore.collection('flashcards').doc(fc.id));
    }
    batch.delete(firestore.collection('packs').doc(id));
    await batch.commit();
  }
  ...
  async incrementPackViews(id: string): Promise<void> {
    ...
    if (pack) {
      await firestore.collection('packs').doc(id).update({ views: (pack.views || 0) + 1 });
    }
  }
  ...
  async createFlashcard(insertFlashcard: InsertFlashcard): Promise<Flashcard> {
    ...
    const flashcard: Flashcard = { id: fcId, ...insertFlashcard } as Flashcard;
  ...
  async updateFlashcard(id: string, flashcard: UpdateFlashcard): Promise<Flashcard | undefined> {
    await firestore.collection('flashcards').doc(id).update(flashcard as any);
  ...
  async createAccountRequest(request: InsertAccountRequest): Promise<AccountRequest> {
    ...
    const accountReq: AccountRequest = { id: reqId, status: 'pending', ...request } as AccountRequest;
  ...
  async getValidMessageRecipients(userId: string, userRole: string): Promise<User[]> {
    ...
    return userRole === 'admin'
      ...
      : allUsers.filter(u => u.role === 'admin' && u.id !== userId);
  }
  ...
  async getUnreadConversations(userId: string): Promise<{ conversationWith: string; unreadCount: number }[]> {
    ...
    return Object.entries(conversations).map(([id, count]) => ({
      conversationWith: id,
      unreadCount: count,
    }));
  }
